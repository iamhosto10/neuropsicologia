...
export function CourseCard({ course }: CourseCardProps) {
  return (
    <Link href={`/cursos/${course.slug}`}>
      <Card className="bg-white pt-0 rounded-[2rem] border-none shadow-sm hover:shadow-lg overflow-hidden group transition-all duration-500 hover:-translate-y-2 cursor-pointer h-full">
        <div className={`relative h-48 ${course.bgColor || "bg-gray-100"}`}>
